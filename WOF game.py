...
#Define the WOFHumanPlayer class
...
#Define the WOFComputerPlayer class
...
def requestPlayerMove(player, category, guessed):
    while True: # we're going to keep asking the player for a move until they give a valid one
        ...
        move = str(move).strip().upper() # convert whatever the player entered to UPPERCASE
...
while True:
    ...
    if wheelPrize['type'] == 'bankrupt':
        ...
        if move.strip() == 'EXIT': # leave the game
            print('Until next time!')
            break
        elif move.strip() == 'PASS': # will just move on to next player
